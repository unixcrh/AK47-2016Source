...
namespace MCS.Dynamics.Web.Services
{
    ...
    {
        [WebMethod]
        public bool ValidateCodeNameExist(string schemaType, string id, string parentID, string currentValue, bool includingDeleted)
        {
            return DEDynamicEntityAdapter.Instance.ExistByCodeName(currentValue, DateTime.Now.SimulateTime());
        ...
        //    List<string> strPinYin = DESnapshotAdapter.Instance.GetPinYin(currentValue);
        ...
        //{
        //    DEContainerAndPermissionCollection containerPermissions = null;
        //    if (TimePointContext.Current.UseCurrentTime && Util.SuperVisiorMode == false)
        //        containerPermissions = DEAclAdapter.Instance.LoadCurrentContainerAndPermissions(Util.CurrentUser.ID, new string[] { appID });
