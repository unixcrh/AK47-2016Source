...
namespace MCS.Dynamics.Web.Dialogs
{
	internal abstract class PropertyEditorSceneAdapter
	{
		...
		private DEContainerAndPermissionCollection containerPermissions = null;
		...
		protected DEContainerAndPermissionCollection ContainerPermissions
		{
			get
			{
				if (this.containerPermissions == null)
				{
					...
					{
						this.containerPermissions = DEAclAdapter.Instance.LoadCurrentContainerAndPermissions(Util.CurrentUser.ID, new string[] { this.ParentID });
					}
				}
		...
		public static PropertyEditorSceneAdapter Create(DESchemaObjectBase schemaObj)
		{
		...
		public static PropertyEditorSceneAdapter CreateByCategory(string category)
		{
			switch (category)
			{
                ...
                case "DynamicEntityField":
                    return new EntityFieldPropertyEditorSceneAdapter();
                case "DynamicEntity":
                    return new EntityFieldPropertyEditorSceneAdapter();
                case "ETLEntity":
                    return new EntityFieldPropertyEditorSceneAdapter();
    ...
    internal sealed class EntityPropertyEditorSceneAdapter : PropertyEditorSceneAdapter
    {
        ...
        {
            //var parentRelation = DESchemaRelationObjectAdapter.Instance.LoadByObjectID(this.ObjectID);
    ...
    internal sealed class EntityFieldPropertyEditorSceneAdapter : PropertyEditorSceneAdapter
    {
        protected override bool IsEditEnabled()
        {
            return this.HasPermission("UpdateChildren");
