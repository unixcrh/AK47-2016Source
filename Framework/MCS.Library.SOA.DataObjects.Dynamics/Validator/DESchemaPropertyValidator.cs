...
namespace MCS.Library.SOA.DataObjects.Dynamics.Validators
{
	...
	/// 表示模式属性的值访问
	/// </summary>
	public class DESchemaPropertyValueAccess : ValueAccess
	{
		private SchemaPropertyValue _PropertyValue = null;

		/// <summary>
		/// 使用指定的<see cref="SchemaPropertyValue"/>实例 初始化<see cref="DESchemaPropertyValueAccess"/>的新实例。
		/// </summary>
		/// <param name="pv">一个<see cref="SchemaPropertyValue"/>对象</param>
		public DESchemaPropertyValueAccess(SchemaPropertyValue pv)
		{
			pv.NullCheck("pv");
		...
		public override object GetValue(object target)
		{
			...
			if (target != null)
			{
				(target is DESchemaObjectBase).FalseThrow("{0}不是SchemaObjectBase类型，不能进行属性值的获取", target.GetType());
