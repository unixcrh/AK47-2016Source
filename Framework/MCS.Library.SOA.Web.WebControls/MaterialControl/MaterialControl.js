...
$HBRootNS.MaterialControl.prototype =
{
    ...
    _prepareDialogUploadFileArguments: function () {
        var information = {};
        ...
        information.newMaterials = newMaterials;

        return information;
    },

    get_preferedDialogUploadFileControlUrl: function () {
        var result = this.get_dialogUploadFileControlUrl();

        if (this._fileSelectMode == $HBRootNS.fileSelectMode.TraditionalSingle)
            result = this.get_dialogUploadFileTraditionalControlUrl();
    ...
    _showUploadDialogBtnClick: function () {
        ...
        var information = this._prepareDialogUploadFileArguments();

        var returnInformation = null;

        var dialogUrl = this.get_preferedDialogUploadFileControlUrl();

        if (this._fileSelectMode == $HBRootNS.fileSelectMode.TraditionalSingle) {
            var arg = "dialogHeight : 450px; dialogWidth : 600px; edge : Raised; center : Yes; help : No; resizable : No; status : No; scroll : no";
            returnInformation = window.showModalDialog(dialogUrl, information, arg);
        }
        ...
        if (returnInformation) {
            ...
            for (var i = 0; i < information.newMaterials.length; i++)
                Array.add(this._materials, information.newMaterials[i]);
