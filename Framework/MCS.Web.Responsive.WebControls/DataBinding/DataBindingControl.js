...
$HGRootNS.ClientBindingDataType = function (element) {
    throw Error.invalidOperation();
}

$HGRootNS.ClientBindingDataType.prototype = {
    None: 0,
    String: 1,
    Number: 2,
    Float: 3,
...
$HGRootNS.ClientBindingDataType.registerEnum($HBRootNSName + ".ClientBindingDataType");

//************************ClientBindingDataType*******************************}

//************************BindingDirection*******************************{

$HGRootNS.BindingDirection = function (element) {
    throw Error.invalidOperation();
}

$HGRootNS.BindingDirection.prototype = {
    None: 0,
    ControlToData: 1,
    DataToControl: 2,
    Both: 3
...
$HGRootNS.BindingDirection.registerEnum($HBRootNSName + ".BindingDirection");

//************************BindingDirection*******************************}

//************************DataBindingControl*******************************{

$HGRootNS.DataBindingControl = function (element) {
    $HGRootNS.DataBindingControl.initializeBase(this, [element]);
...
$HGRootNS.DataBindingControl.prototype = {
    initialize: function () {
        Sys.Application.add_load(this._applicationLoad$delegate);

        $HGRootNS.DataBindingControl.callBaseMethod(this, 'initialize');
    ...
    _formatDataByType: function (data, item) {
        ...
        if (item.Format) {
            switch (item.ClientDataType) {
                case $HGRootNS.ClientBindingDataType.Number:
                case $HGRootNS.ClientBindingDataType.Float:
                    result = $HGRootNS.Formatter.pictureFormat(data, item.Format);
                    break;
                case $HGRootNS.ClientBindingDataType.DateTime:
    ...
    _changeStringToTargetType: function (str, targetType) {
        ...
        switch (targetType) {
            ...
            case $HGRootNS.ClientBindingDataType.Float:
                {
                    str = str.replace(/,/g, '');
                    result = (targetType == $HGRootNS.ClientBindingDataType.Number) ? parseInt(str) : parseFloat(str);
    ...
    bindControl: function (control, dataType, formatString) {
        var binder = new $HGRootNS.TextBoxValidationBinder();
    ...
    checkAllData: function (groupID) {
        ...
        this.errorMessages = new Array();
        var allValues = this._get_allControlPropertyValues();

        for (var k = 0; k < this.validControls.length; k++) {

            var doValidate = true;
            var validator = this.validControls[k][1];
            var targetControl = this.validControls[k][0];

            if (doCompareGroupID)
                doValidate = (validator.ValidationGroup & groupID) != 0;
            ...
            if (doValidate) {
                ...
                if (validator.Func) {
                    if (!validator.Func()) {
                        ...
                        this.errorMessages.push(validator.errorMsg);
                    }
                }
                else
                    if (this.validateData(
                                allValues[validator.DataPropertyName],
                                validator,
                                allValues) == false) {
                        tempisValidate = false;
                    }

                if (tempisValidate == false) {
                    isValidate = false;
                }
            }
        }
    ...
    _get_allControlPropertyValues: function () {
        var result = {};

        for (var i = 0; i < this.validControls.length; i++) {
            var targetControl = this.validControls[i][0];
            var validator = this.validControls[i][1];

            result[validator.DataPropertyName] = this._get_controlPropertyValue(targetControl, validator);
        }
    ...
    validateData: function (cvalue, validator, allValues) {
        var isValidate = false;
        ...
        if (validator.IsAnd) {
            isValidate = this.andValidateData(cvalue, validator.CvtList, allValues);
        }
        else {
            isValidate = this.orValidateData(cvalue, validator.CvtList, allValues);
        }

        return isValidate;
    },

    orValidateData: function (cvalue, vadlist, allValues) {
        ...
        for (var i = 0; i < vadlist.length; i++) {
            if (typeof (vadlist[i].ClientValidateMethodName) != "undefined" && vadlist[i].ClientValidateMethodName.length > 0) {
                ...
                if (typeof (method) != "function") {
                    ...
                    if (methodInstance.validate(cvalue, vadlist[i].AdditionalData, allValues))
                        return true;
                    else {
    ...
    andValidateData: function (cvalue, vadlist, allValues) {
        for (var i = 0; i < vadlist.length; i++) {
            if (typeof (vadlist[i].ClientValidateMethodName) != "undefined" && vadlist[i].ClientValidateMethodName.length > 0) {
                var method = $HGRootNS.ValidatorManager[vadlist[i].ClientValidateMethodName];
                if (typeof (method) != "function") {
...
$HGRootNS.DataBindingControl.checkBindingControlDataByGroup = function (groupID) {
    var isValidate = true;
...
$HGRootNS.DataBindingControl.checkBindingControlData = function (val, args) {
    var result = $HGRootNS.DataBindingControl.checkBindingControlDataByGroup();
...
$HGRootNS.DataBindingControl.registerClass($HBRootNSName + ".DataBindingControl", $HGRootNS.ControlBase);
