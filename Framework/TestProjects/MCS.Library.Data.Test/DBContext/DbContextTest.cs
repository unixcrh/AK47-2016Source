...
namespace MCS.Library.Data.Test.DBContextTest
{
    ...
    public class DbContextTest
    {
        ...
        {
            using (DbContext context = DbContext.GetContextAsync(Common.ConnectionName).Result)
            {
        ...
        {
            using (DbContext contextOuter = await DbContext.GetContextAsync(Common.ConnectionName))
            {
                ...
                using (DbContext contextInner = await DbContext.GetContextAsync(Common.ConnectionName))
                {
        ...
        private static async Task<DbConnection> CascadeTransactionScopeAndDbContext()
        {
            ...
            {
                using (DbContext context = await DbContext.GetContextAsync(Common.ConnectionName))
                {
